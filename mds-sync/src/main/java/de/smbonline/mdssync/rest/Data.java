...
public class Data {
     ...
     * parsing given string fails. Returns an empty instance if given string is empty or blank.
     *
     * @param json json string
     * @return Data object
     */
    public static Data fromJsonSuppressExceptions(final @Nullable String json) {
        if (StringUtils.isBlank(json) || "{}".equals(json)) {
            return new Data();
        }
        try {
            return JSON_HELPER.readValue(json, Data.class);
        } catch (Exception exc) {
            return new Data().setAttribute("error", exc.getMessage());
        }
    }
    ...
    public Data setAttribute(final String attribute, final @Nullable Object value) {
        ...
        if (value instanceof Collection<?> values && !values.isEmpty()) {
            ...
            // convert list of map-like types to list of Data objects
            if (firstValue instanceof Map<?, ?>) {
                Collection<Data> attrValues = new LinkedHashSet<>();
                for (Map<String, ?> nested : (Collection<Map<String, ?>>) values) {
                    attrValues.add(fromMap(nested));
                }
                attrValue = attrValues;
            }
            // else: keep list of scalar values
        }
    ...
    @SuppressWarnings("unchecked")
    private static @Nullable <T> T getNestedTypedAttributeRecursive(final String attributePath, final Data dto) {
     ...
     * @return this
     */
    public Data setNestedAttribute(final String attributePath, @Nullable final Object value) {
        setNestedAttributeRecursive(attributePath, value, this);
        return this;
    }

    private static void setNestedAttributeRecursive(final String attributePath, final @Nullable Object value, final Data dto) {
        int index = requireValidIndexOfSeparator(attributePath);
        if (index < 0) {
     ...
     * @return json string
     */
    public static String toJsonSuppressExceptions(final Data obj) {
        try {
