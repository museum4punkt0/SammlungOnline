...
import { IConfiguration } from '../../config/interfaces/config.interface';
import { SmbTopics, SmbTopicsObjects } from '../../config/generated/graphql';
...
import TopicsRepository from '../../graphQL/topics/topics.repository';
...
class TopicsService {
  ...
  public getTopics(): {
    ...
    const { loading, error, data } = this._topicsRepository.fetchTopics(
  ...
  public findTopicsInfoByExhibitId(exhibitId: number) {
    ...
    const { loading, error, data } = this._topicsRepository.fetchTopicsByExhibitId(
      exhibitId,
      language,
    );
    ...
    return topics.map((topic) => {
      return {
        ...
        subtitle: topic.topics_translations[0].description
          ...
          : '',
        previewImageCard: this._imageUrlBuilder.buildUrl(
          topic.preview_image,
          imageSize,
          imageSize,
        ),
