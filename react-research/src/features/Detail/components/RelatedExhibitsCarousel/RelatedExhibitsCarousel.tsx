...
const RelatedExhibitsCarousel: React.FC<IRelatedExhibitsCarouselProps> = ({ exhibit }) => {
        ...
        <Box paddingBottom={10}>
            ...
            <Carousel color="#000" cellSpacing={16} slidesToShow={slidesToShow}>
                {relatedObjectsWithImages.map((item, index) => {
                    return <CarouselImageCard key={index} img={item.img} link={item.link} caption={item.caption} />;
                })}
            </Carousel>
        </Box>
    );
