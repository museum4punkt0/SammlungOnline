...
import { MISSING_IMAGES_WITH_REASONS } from '../../components/ExhibitPreview/ExhibitPreview';

import { useQueryParamsGetter } from '../../hooks/use-query-params-getter.hook';
...
const DetailPage: React.FC = () => {
  ...
  // if exhibit doesn't have title but has technical term or title is the same
  // as technical term we should display technical term as title with working
  // internal link
  const rawTechnicalTerm = exhibit?.technicalTerm ?? '';
  let term = typeof rawTechnicalTerm === 'string' ? rawTechnicalTerm : '';
  let title = exhibit?.title ?? '';
  if (!title || title === term) {
    title = term;
    term = '';
  }
  if (rawTechnicalTerm && typeof rawTechnicalTerm !== 'string') {
    term = replaceInternalLinks(
      'technicalTerm',
      rawTechnicalTerm.markup,
      rawTechnicalTerm.formatted,
      t,
      getQueryParamsString,
    );
    if (exhibit?.title === rawTechnicalTerm.formatted || !exhibit?.title) {
      title = term;
      term = '';
    }
  }
  ...
  const allowImageActions = (): boolean => {
    ...
    if (MISSING_IMAGES_WITH_REASONS.includes(currentAttachment?.id as number)) {
      return false;
    }
    if (license) {
      return (
  ...
  const resizedCurrentAttachment = (w = 1000, h = 600) => {
    if (
      ...
      !MISSING_IMAGES_WITH_REASONS.includes(currentAttachment.id as number)
    )
  ...
  const metaDescription = exhibit?.description?.formatted?.length
    ? exhibit.description.formatted.substring(0, 300).replace(/["]/g, '')
  ...
  const metaImage =
    ...
    !MISSING_IMAGES_WITH_REASONS.includes(attachments[0].id as number)
      ? imageUrlBuilder.buildUrl(attachments[0].filename, 1200, 630)
      : undefined;
