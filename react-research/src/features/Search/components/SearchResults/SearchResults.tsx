...
import { ExhibitModel, LoadingSpinner } from 'smb-react-components-library';
...
const SearchResultsModule: React.FC<ISearchResultsModule> = (props) => {
    ...
    const defaultView =
        ((localStorage.getItem('paginationView') as unknown) as ESearchResultView) ?? ESearchResultView.GRID;
    ...
    const toggleViewStyle = () => {
        const newView = paginationView === ESearchResultView.GRID ? ESearchResultView.LIST : ESearchResultView.GRID;
    ...
    return (
        <>
            ...
            <div className={classes.content}>
                <SearchContent data={dataWithImages} view={paginationView} onClick={onItemSelect} />
            </div>
            <SearchPagination
                limit={limit}
                total={total}
