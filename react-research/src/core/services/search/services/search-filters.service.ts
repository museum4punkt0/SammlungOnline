...
import { ISearchFilter } from '../interfaces/filters/filter.interface';
...
class SearchFiltersService {
    ...
    private _createAdvancedFilters(advancedFilters: IVirtualSearchFilterGroup[] = []): ISearchFilter[] {
        const filters: ISearchFilter[] = [];

        advancedFilters?.forEach((advancedFilter) => {
            const selectedFilters = advancedFilter.filters.filter((filter) => filter.virtualValue);
            if (selectedFilters.length) {
                ...
                selectedFilters.forEach((filter) => {
                    ...
                    if (selectedOptions.length) {
                        value = selectedOptions.map((option) => option.value).join(` ${ESearchOperators.OR} `);
                    }
                    values.push(value);
                });
    ...
    private _createConditionsFilters(conditions: IVirtualSearchAttributeCondition[] = []): ISearchFilter[] {
    ...
    private _createSearchControlsFilters(searchControls: IVirtualSearchSwitch[] = []): ISearchFilter[] {
