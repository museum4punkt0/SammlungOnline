...
import { EGraphqlTranslationAttributesFields } from 'src'
...
export class ExhibitRepository {
  private readonly _attributesToBeFetched = Object.values(
  ...
  public fetchById(
    ...
    lang: string,
    skip = false,
  ): IRepositoryResponse<SmbObjects | null> {
    const {
      loading,
      error,
      data: response,
    } = useQuery<QueryRoot>(FetchExhibitById, {
      variables: {
        object_id: exhibitId,
