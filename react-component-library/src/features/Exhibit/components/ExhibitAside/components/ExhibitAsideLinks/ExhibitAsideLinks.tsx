...
import { IExhibitAsideLinksProps } from '../../types';

export const ExhibitAsideLinks: React.FC<IExhibitAsideLinksProps> = ({
  title,
  links,
  type,
}) => {
  ...
  return (
    <div className={clsx(classes.content, containerTypeClasses)}>
      <Typography
        component="div"
        variant="h5"
        className={clsx(classes.typoTitle, containerTypeClasses)}
      >
        {title}
      </Typography>
      {links?.map((link, i) => (
        <Link
          ...
          href={link.href}
          color="inherit"
          className={clsx(classes.link, containerTypeClasses)}
        >
          {link.caption}
          <ArrowRightAltIcon />
        </Link>
