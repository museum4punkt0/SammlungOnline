...
import { MediaPlayerContext, MediaPlayerControls, Progressbar } from '../../../index';
import { Slide, SliderModule } from '../../../Slider/components';
import { CollectionObject } from 'src/features/collection/types';
import { MediaPlayerContextData } from '../MediaPlayerContext/MediaPlayerContext';
import useLazyLoadImage from '../../../../components/LazyLoadImage/hooks/use-lazy-load-Image.hook';
import { ImageZoomModal } from '../../../../index';
...
function generateImageUrls(
  ...
  return collectionObjects.map((value) =>
    value.imageUrlBuilder
      .buildUrl(value.imageId, maxPreviewImageSize, maxPreviewImageSize)
      .toString(),
  );
}
...
export function MediaPlayer({
  ...
  const mediaPlayerContextData =
    useContext<MediaPlayerContextData>(MediaPlayerContext);
  ...
  useEffect(() => {
    setPreviewImages(
      generateImageUrls(
        mediaPlayerContextData.collectionObjects,
        maxPreviewImageSize,
      ),
    );
  ...
  const togglePlayer = (event: React.MouseEvent, fullscreen: boolean): void => {
    mediaPlayerContextData.toggleFullscreen &&
      mediaPlayerContextData.toggleFullscreen(fullscreen);
        ...
        <div className={classes.childWrapper}>
          ...
          {!mediaPlayerContext.open && (
            <div>
              ...
              />
              <MediaPlayerControls
                data-testid={'media-player-button-wrapper'}
                playing={false}
            ...
            <Fullscreen
              ...
              onChange={(isFullscreen: boolean): void => {
                if (!isFullscreen && mediaPlayerContext.fullscreen) {
                  mediaPlayerContext.toggleFullscreen &&
                    mediaPlayerContext.toggleFullscreen();
                }
              }}
            >
              {mediaPlayerContext.showControls && (
                <IconButton
                  data-testid={'media-player-back-button'}
              ...
              >
                {mediaPlayerContext.collectionObjects.map(
                  (collectionObject, index) => (
                    <Slide
                      key={index}
                      image={collectionObject.imageUrlBuilder
                        .buildUrl(
                          collectionObject.imageId,
                          maxImageSize,
                          maxImageSize,
                        )
                        .toString()}
                      slideClasses={clsx(classes.slide, slideClasses, {
                        [classes.slideFullscreen]:
                          mediaPlayerContext.fullscreen,
                      })}
                      onClick={(): void =>
                        mediaPlayerContext.toggleControls &&
                        mediaPlayerContext.toggleControls()
                      }
                      onMouseMove={(): void =>
                        mediaPlayerContext.toggleControls &&
                        mediaPlayerContext.toggleControls(true)
                      }
                    />
                  ),
                )}
              ...
              {mediaPlayerContext.collectionObjects.length > 0 && (
                <div className={classes.titleWrapper}>
                  <div
                    className={clsx(classes.titleElements, {
                      [classes.titleHide]: !showTitle,
                    })}
                  >
                    <Typography
                      component="div"
                    ...
                    >
                      {
                        mediaPlayerContext.collectionObjects[
                          mediaPlayerContext.activeIndex
                        ].title
                      }
                    </Typography>
                    <ButtonBase
              ...
              <div
                className={clsx(classes.mediaPlayerControlsWrapper, {
                  [classes.mediaPlayerControlsWrapperHide]:
                    !mediaPlayerContext.showControls,
                })}
              >
                <Progressbar
                ...
                <MediaPlayerControls
                  ...
                  onPlayPauseButtonClick={
                    mediaPlayerContext.togglePlayerPlaying
                  }
