  ...
  query FetchGuide($id: bigint!, $attribute: String!, $lang: String!) {
    smb_tours(where: { id: { _eq: $id } }) {
      ...
      tours_objects(order_by: { sequence: asc }) {
        ...
        room
        object {
          attribute_translations(where: { attribute_key: { _eq: $attribute }, language: { lang: { _eq: "de" } } }) {
            attribute_key
            value
          }
...
class GuideRepository {
  ...
  fetchObjects(ids: number[], lang: string): { loading: boolean; error: ApolloError | undefined; data: SmbObjects[] | null } {
