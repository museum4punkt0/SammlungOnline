...
export const Footer: React.FC<IFooterProps> = ({ configuration }) => {
  ...
  const links = {
    ...
    col4: [
      ...
      { text: t('footer.contact'), href: 'mailto: smb-digital@smb.spk-berlin.de' },
    ],
  };
  ...
  return (
    <Container className={classes.wrapper} disableGutters={true} maxWidth='lg'>
      <FooterGrid className={classes.footerGrid}>
        <Paper className={classes.footerCard}>
          {links.col1.map((link, i) => (
            <React.Fragment key={i}>
              <Link className={classes.link} data-cy={`footer-link-col1-${i}`} href={link.href} color={'inherit'}>
        ...
        <Paper className={classes.footerCard}>
          {links.col4.map((link, i) => (
            <React.Fragment key={i}>
              <Link className={classes.link} data-cy={`footer-link-col4-${i}`} href={link.href} color={'inherit'}>
                <Typography variant={'caption'} color={'inherit'}>
                  {link.text}
                </Typography>
              </Link>
      ...
      <Typography display='block' align='right' variant='body1' style={{ color: 'white' }}>
        {copyright}
      </Typography>
    </Container>
  );
