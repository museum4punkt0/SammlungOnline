...
class GuideService {
  ...
  getGuide(
    ...
    const {
      ...
      error: objectError,
      data: objectData,
    } = this.guideRepository.fetchObjects(objectIds, lang);
  ...
  // todo translations
  private convertGuideToTourData(
    tour: SmbTours,
    imageCardSize: number,
    imageSliderSize: number,
  ): ITourData {
      ...
      tour.tours_objects.forEach((obj) => {
        if (obj) {
          ...
          if (obj?.object?.attachments[0]?.attachment) {
            img_url = this.buildImageUrl(
              obj.object.attachments[0].attachment,
              imageCardSize,
            );
          }
          ...
          if (obj?.sequence && obj?.room) {
            if (
              ...
              stations[stations.length - 1].name === obj.room
            ) {
              ...
              if (dir[dirCount]) {
                while (
                  dirCount < dir.length &&
                  dir[dirCount].indexOf('Weiter zu') === -1
                ) {
  ...
  getGuides(
    ...
    const { loading, error, data } = this.guideRepository.fetchGuides(
      LanguageService.getCurrentLanguage(),
    );
    let contextData: Array<ICollectionContextData> = [];

    if (!loading && data) {
      contextData = this.convertGuidesToCollectionsContext(
        data,
        imageCardSize,
        ImageSliderSize,
      );
  ...
  private convertGuidesToCollectionsContext(
    ...
    for (const tour of tours) {
      const collectionData = this.convertGuidesToCollectionContext(
        tour,
        imageCardSize,
        ImageSliderSize,
      );
  ...
  private convertGuidesToCollectionContext(
    tour: SmbTours,
    imageCardSize: number,
    imageSliderSize: number,
  ): ICollectionContextData | null {
