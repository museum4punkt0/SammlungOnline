...
import {
  FieldVisibility,
  Visibility,
} from '../ObjectDescriptionModule/ObjectDescriptionModule';
...
function ObjectDescriptionAccordion({
  config,
}: {
  ...
  return (
    <div className={clsx(classes.spacingTop, classes.accordionWrapper)}>
      {config.map((f) => renderRow(f))}
    </div>
  );
}
