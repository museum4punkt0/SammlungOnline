...
const ToursObjectAssignmentToolbar = ({
    onCancel,
    ...props
}: {
    onCancel: () => void;
...
const ObjectSelectInput = (): ReactElement => {
    ...
    const filter = (searchText: string): void => {
        ...
        getSelectOptions(searchText, dataProvider).then((value) => {
            ...
            // @ts-ignore
            document.getElementById('object_id').value = searchText;
        });
    };
...
function ToursObjectAssignmentButton(record: Record<string, any>): ReactElement {
    const [showDialog, setShowDialog] = useState<boolean>(false);
    const [create] = useCreate('smb_tours_objects');
    const notify = useNotify();
    const refresh = useRefresh();
    const toursId: number = parseInt(record.record.id, 10);

    const handleSubmit = async (values: Record<string, any>) => {
        values.tour_id = toursId;
        return create(
            { payload: { data: values } },
            {
                onSuccess: () => {
    ...
    return (
        <>
            ...
            <Dialog fullWidth={true} open={showDialog} onClose={() => setShowDialog(false)}>
                <DialogTitle>Füge weitere Objekte der Tour hinzu</DialogTitle>
                <DialogContent>
                    <SimpleForm
                        resource={'smb_tours_objects'}
                        toolbar={<ToursObjectAssignmentToolbar onCancel={() => setShowDialog(false)} />}
                        save={handleSubmit}
                    >
                        {/* <ObjectSelectInput /> */}
                        <TextInput source={'object_id'} label={'field.object_id'} />
                        <TextInput source={'room'} label={'field.room'} />
                        <TextInput source={'sequence'} label={'field.sequence'} validate={required()} />
                    </SimpleForm>
                </DialogContent>
            </Dialog>
        </>
...
export default ToursObjectAssignmentButton;
