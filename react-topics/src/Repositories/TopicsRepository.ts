            ...
            objects {
                object {
                    ...
                    attribute_translations(where: { 
                        attribute_key: { _eq: "${config.DATA_CONFIG.OBJECT_ATTRIBUTE_KEY_TITLE}" }, 
...
class TopicsRepository {
    ...
    fetchTopic(id: number, lang: string): { loading: boolean; error: ApolloError | undefined; data: SmbTopics | null } {
        const { loading, error, data } = useQuery<QueryRoot>(FetchTopic, { variables: { id: id, lang: lang } });
        let resultData = null;
