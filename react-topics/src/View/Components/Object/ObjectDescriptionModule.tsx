...
import React, { ReactElement, ReactHTMLElement, useContext } from 'react';
import ObjectActions from './ObjectActions';
import ObjectContext, { ObjectContextData } from './ObjectContext';
import ObjectDescriptionAccordion from './ObjectDescriptionAccordion';
import ObjectDescriptionAside from './ObjectDescriptionAside';
import ObjectDescriptionGrid from './ObjectDescriptionGrid';
...
function ObjectDescriptionModule({
    ...
    const title = (objectContextData.objectData?.objectTitles && objectContextData.objectData.objectTitles[0]) || '';
    const term = objectContextData.objectData?.technicalTerm || '';
    const date = (objectContextData.objectData?.dating && objectContextData.objectData?.dating[0]) || '';
        ...
        mainTitles.length > 0
            ? mainTitles.map((mainTitle: string, index: number) => (
                  <Typography
                      key={index}
                      variant={'h3'}
                      className={clsx(classes.contrastText, classes.largeText, classes.boldText)}
                  >
                      {mainTitle}
                  </Typography>
              ))
        ...
        <div id={'DescriptionModule'} className={classes.container}>
            ...
            >
                ...
                <Grid item={true} xs={12} sm={12} lg={1} md={2}>
                    <ObjectActions
                        showImageActions={objectContextData?.objectData?.hasPrimaryImage || false}
