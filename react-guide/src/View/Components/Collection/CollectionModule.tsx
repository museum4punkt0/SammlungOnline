import React, { createRef, ReactElement, RefObject, useContext, useState } from 'react';
import CollectionContext, { CollectionContextData } from './CollectionContext';
...
function CollectionModule({
    ...
    const [contentContainerRef] = useState<RefObject<HTMLDivElement>>(createRef<HTMLDivElement>());
    ...
    const imageClicked = (index: number): void => {
        collectionContext.onCollectionObjectSelected && collectionContext.onCollectionObjectSelected(index);
        scrollToTop();
    };
        ...
        <CollectionContext.Consumer>
            {(collectionContext): ReactElement => (
                <div className={classes.contentWrapper} ref={contentContainerRef}>
                    <Grid container={true} spacing={1} direction={'row'} justify={'center'} alignItems={'center'}>
                        {title && (
                            <Grid item={true} xs={12} className={classes.titleGridItem}>
                                <ButtonBase onClick={discoverButtonClicked} className={classes.buttonTitle}>
                                    <span>
                                        <Typography variant={'h3'} className={classes.iconButtonTitleTypo}>
                                            {title}
                                        </Typography>
                        ...
                        {collectionContext.collectionObjects.map((collectionObject, index) => (
                            <Grid item={true} xs={12} sm={6} md={4} lg={3} key={index}>
                                <Card className={classes.card}>
                                    <LazyImage src={buildImageUrl(collectionObject, maxImageSize)}>
                                        <CardMedia
                                    ...
                                    <CardContent className={classes.cardContent}>
                                        <IconButton onClick={(): void => objectInfoClicked(collectionObject.objectId)}>
                                            <InfoOutlinedIcon />
                                        </IconButton>
                                        <Typography variant={'body1'} className={classes.cardTypo}>
                        ...
                        {bottomTitle && (
                            <Grid item={true} xs={12} className={classes.titleGridItem}>
                                <ButtonBase onClick={scrollToTop} className={classes.buttonTitle}>
                                    <span>
                                        <ArrowRightAltOutlinedIcon
