import { Button, Card, Checkbox, Grid, IconButton, Typography } from '@material-ui/core';
...
function CollapsibleCardHeader({
    ...
    const [checked, setChecked] = React.useState(window.sessionStorage.getItem(checkboxStorage) != null ? window.sessionStorage.getItem(checkboxStorage) === "true" : false);
    ...
    const handleCheck = (event: { target: { checked: React.SetStateAction<boolean>; }; }) => {
        setChecked(event.target.checked);
        sessionStorage.setItem(checkboxStorage, event.target.checked ? "true" : "false");
    };

    return (
        <Card
            elevation={0}
            style={{ backgroundColor: 'white', width: '100%', height: '100%', borderRadius: '0px' }}
        >
                ...
                <Grid item container xs={2} justify='flex-end' wrap='nowrap'>
                    ...
                    {share && (
                        <Grid item container alignContent='center' justify='center'>
                            ...
                            >
                                <ShareOutlinedIcon color='primary' className={classes.iconHover} />
                            </IconButton>
                    ...
                    {/* chackbox */}
                    {checkbox && (
                        <Grid item container alignContent='center' style={{ height: '3.125rem', marginRight: '.5rem' }}>
                            <Grid item container justify='flex-end'>
                                <button onClick={toggleCheck} className={classes.checkboxButton}>
                                    <Checkbox
                                        tabIndex={-1}
