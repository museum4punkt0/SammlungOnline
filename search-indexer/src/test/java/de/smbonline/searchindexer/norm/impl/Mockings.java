...
public final class Mockings {
    ...
    static {
        ...
        ORGUNIT_COMPILATION_MAPPING.put("MKMittelalterFruhmittelalter476bis900", "Mittelalter | Frühmittelalter (476 bis 900)");
    ...
    public static GraphQlService mappingSupplierMock() {
        GraphQlService service = Mockito.mock(GraphQlService.class);
        Mockito.when(service.fetchBuildings()).thenReturn(buildings());
        Mockito.when(service.fetchCollections()).thenReturn(collections());
        Mockito.when(service.fetchCompilations()).thenReturn(compilations());
        Mockito.when(service.fetchAssortments(Mockito.any())).thenReturn(Collections.emptyList());
        return service;
    }
    ...
    public static ObjectProvider<GraphQlService> graphQlProvider(final GraphQlService mockService) {
        return new ObjectProvider<>() {
            @Override
            public GraphQlService getObject(Object... args) throws BeansException {
                return mockService;
            }

            @Override
            public GraphQlService getIfAvailable() throws BeansException {
                return mockService;
            }

            @Override
            public GraphQlService getIfUnique() throws BeansException {
                return mockService;
            }

            @Override
            public GraphQlService getObject() throws BeansException {
                return mockService;
            }
        };
    }
